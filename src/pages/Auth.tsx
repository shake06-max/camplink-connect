...
export default Auth;    setBusy(true);
    try {
      if (mode === "signup") {
        const { error } = await supabase.auth.signUp({
          email, password,
  ...
  return (
    <div className="min-h-screen gradient-hero flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-3xl bg-card/95 backdrop-blur-xl shadow-glow border border-border p-8 animate-fade-in">
        <div className="flex flex-col items-center mb-6">
          <Logo size={56} withText={false} />
          <h1 className="text-2xl font-extrabold mt-4">Welcome to Camplink</h1>
          <p className="text-sm text-muted-foreground mt-1">Your campus marketplace & community</p>
        </div>
        ...
        <Tabs value={tab} onValueChange={setTab}>
          ...
          <div className="space-y-3 mt-4">
            <div><Label>Email</Label><Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" /></div>
            <div><Label>Password</Label><Input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="At least 8 characters" /></div>
            <Button className="w-full gradient-accent shadow-glow" onClick={() => handle(tab as any)} disabled={busy}>
              {busy && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
              {tab === "signin" ? "Sign in" : "Create account"}
            </Button>
            {tab === "signin" && (
        ...
        <div className="relative my-5"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center text-xs"><span className="bg-card px-2 text-muted-foreground">or continue with</span></div></div>
